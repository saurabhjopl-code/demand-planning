// =====================================================
// SUMMARY SECTION (1 → 6) — FINAL, CORRECT & LOCKED
// =====================================================
...
document.addEventListener("google-ready", async () => {
  try {
    ...
    /* =====================================================
       STYLE STATUS MAPS (USED ONLY IN SUMMARY 5 & 6)
    ===================================================== */
    ...
    saleData.forEach(r => {
      const m = N(r["Month"]);
      const u = Number(r["Units"]) || 0;
      monthSaleMap[m] = (monthSaleMap[m] || 0) + u;
    });
    ...
    stockData.forEach(r => {
      ...
      const u = Number(r["Units"]) || 0;
      if (!fc) return;
      fcStockMap[fc] = (fcStockMap[fc] || 0) + u;
    });
    ...
    saleData.forEach(r=>{
      ...
      sizeCategorySaleMap[cat] = (sizeCategorySaleMap[cat]||0)+u;
    });
    ...
    sizeOrder.forEach(sz=>{
      ...
      const cat = sizeCategory(sz);
      const sizeShare = totalSaleUnits ? ((sold/totalSaleUnits)*100).toFixed(2) : "0.00";
      const catShare = totalSaleUnits ? ((sizeCategorySaleMap[cat]/totalSaleUnits)*100).toFixed(2) : "0.00";
      s4 += `<tr>
        <td>${sz}</td>
        <td>${cat}</td>
        <td>${sold}</td>
    ...
    s5 += `</table>`;
    summary5.innerHTML = s5;

    /* =====================================================
       SUMMARY 6 – CATEGORY WISE SALE
       (FROM STYLE STATUS → Category)
    ===================================================== */
